-- PlayerMaskOff = casing / unmasked. Vanilla blocks sprint/crouch/jump; we restore them.
local level_id = Global.level_data and Global.level_data.level_id or ""

-- Crouch + jump (vanilla only shows the block hint).
...
-- Sprint input (same rules as standard movement).
...
-- Keep mask-off detection unless the map is hostile or we just interacted suspiciously.
...
-- Start sprint without relying on an equipped weapon.
...
-- Allow ability throwables (Pocket ECM, etc.); real grenades stay blocked in casing.
Hooks:PostHook(PlayerMaskOff, "_update_check_actions", "CheckAbilityItem", function(self, t, dt)
	local projectile_entry = managers.blackmarket:equipped_projectile()
	local projectile_tweak = projectile_entry and tweak_data.blackmarket.projectiles[projectile_entry]

	if projectile_tweak and projectile_tweak.ability then
		self:_check_action_use_ability(t, self:_get_input(t, dt))
	end
end)
